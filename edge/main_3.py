...
# SHARED STATE
# =====================================
latest_frame    = None  # frame mentah dari RTSP
annotated_frame = None  # frame + bounding box untuk dashboard
...
def resource_monitoring_thread():
    ...
    while True:
        try:
            ...
            cap_check.release()
            rtsp_status = "up" if ret else "down"
            if not ret:
                reconnect_count += 1
        except Exception:
            rtsp_status     = "down"
        ...
        print(
            ...
            f"FPS:{round(fps_rtsp,2)} | LATENCY:{latency}ms | "
            f"EDGE:{edge_status}"
        )
...
# =====================================
# MAIN: APD DETECTION (tanpa imshow)
# =====================================
def main():
